...
const getAttendanceStatus = (percent: number) => {
  if (percent >= 85) return { color: "bg-green-500", status: "excellent", icon: TrendingUp, textColor: "text-green-600" };
  if (percent >= 75) return { color: "bg-blue-500", status: "good", icon: Minus, textColor: "text-blue-600" };
  if (percent >= 60) return { color: "bg-orange-500", status: "warning", icon: TrendingDown, textColor: "text-orange-600" };
  return { color: "bg-red-500", status: "critical", icon: TrendingDown, textColor: "text-red-600" };
};
    ...
    <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-gray-900">{safeItems.length}</div>
      ...
      <div className="space-y-3">
        {safeItems.map((item) => {
          ...
          return (
            <div key={item.courseCode} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-sm font-semibold text-gray-900 truncate">{item.courseCode}</h3>
